...
//  MailReceipt
//
//  Created by Gyorvari Gabor on 2016. 09. 27..
...
+ (NSDictionary *)commonHooks {
    return @{
        @"MFMessageContext": @[
            @"async_setFlagWithKey:state:forMessages:reason:",
            @"async_setFlagsFromDictionary:forMessages:"
        ]
    };
}
...
+ (void)injectUsingMethodPrefix:(NSString *)prefix hooks:(NSDictionary*)hooks{
    NSString *extensionClassSuffix = @"MailDisableAutoRead";
    ...
    for(NSString *class in hooks) {
        ...
        if(!mailClass) {
            NSLog(@"WARNING: Class %@ doesn't exist. This leads to unexpected behaviour!", class);
            continue;
        }
        ...
        if(extend) {
            if(![mailClass jrlp_addMethodsFromClass:extensionClass error:&error])
                NSLog(@"WARNING: methods of class %@ couldn't be added to %@ - %@", extensionClass, mailClass, error);
        }
        ...
        for(id selectorNames in selectors) {
            ...
            error = nil;
            NSString *extensionSelectorName = [NSString stringWithFormat:@"%@%@%@", prefix, [[gmSelectorName substringToIndex:1] uppercaseString], [gmSelectorName substringFromIndex:1]];
            ...
            if(![mailClass jrlp_swizzleMethod:selector withMethod:extensionSelector error:&error]) {
                ...
                if(![mailClass jrlp_swizzleClassMethod:selector withClassMethod:extensionSelector error:&error])
                    NSLog(@"WARNING: %@ doesn't respond to selector %@", NSStringFromClass(mailClass),NSStringFromSelector(selector));
            }
        }
    }
}
